import {retainedMessages} from "./chat-retention.js";
...
export function containsContactDetails(value){const text=String(value??"");const digits=text.replace(/[^0-9]/g,"");return /(?:\+?52[\s.()-]*)?(?:\d[\s.()-]*){10,}/.test(text)||/(?:\bwhats\s*app\b|\bwa\.me\/|\btelegram\b|https?:\/\/|www\.|@)/i.test(text)||digits.length>=10&&/\b(?:cel|tel[eé]fono|n[uú]mero|ll[aá]mame|contacto)\b/i.test(text)}
...
export function orderMessages(order){return retainedMessages(order?.messages).filter(x=>ROLES.includes(x.role)&&typeof x.body==="string")}

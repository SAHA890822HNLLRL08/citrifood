...
const defaults={
"Tacos El Centro":[{id:"tacos-2x1",title:"2x1 en tacos de bistec",description:"Llévate dos tacos por el precio de uno.",name:"Promo 2x1 · Tacos de bistec",price:75,emoji:"🌮",active:true,sponsored:true}],
"Burger House":[{id:"combo-citri",title:"Combo Citri",description:"Combo especial de Burger House.",name:"Combo Citri",price:129,emoji:"🍔",active:true,sponsored:true}],
...
export function sponsoredPromotions(){return Object.entries(loadPromotions()).flatMap(([restaurant,list])=>list.filter(p=>p.active!==false&&p.sponsored===true).map(p=>({...p,restaurant})))}
export function setPromotionSponsored(restaurant,id,sponsored){const current=loadPromotions(),entries=current[restaurant]||[];if(!entries.some(p=>p.id===id))return false;savePromotions({...current,[restaurant]:entries.map(p=>p.id===id?{...p,sponsored:!!sponsored}:p)});return true}

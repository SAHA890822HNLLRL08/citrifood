...
### Bandeja de restaurante para el piloto entre celulares
La migración `supabase/migrations/202609230004_restaurant_pilot.sql` permite que un administrador asigne cuentas verificadas a nombres exactos de restaurantes. Solo el administrador puede insertar membresías; la consulta de pedidos se protege mediante RLS. `GET /api/restaurant/orders` devuelve una bandeja de pedidos filtrada por los restaurantes autorizados; la pantalla `/piloto` la actualiza cada 10 segundos. Para una prueba: entrar con una cuenta de cliente y otra cuenta autorizada como restaurante en dos celulares, crear un pedido ficticio con el nombre exacto asignado y comprobar su aparición en la bandeja. **No se ha aplicado la migración ni conectado un Supabase real.** No hay aceptación de pedidos, cobros ni despacho entre teléfonos todavía. La bandeja omite direcciones por defecto para reducir exposición de datos en esta etapa.

### Primer cambio de estado entre dos teléfonos (aún sin desplegar)
La migración `202609230005_restaurant_status.sql` agrega una función transaccional para que **solo personal asignado** acepte pedidos `Nuevo → Preparando` y marque `Preparando → Listo`. `POST /api/restaurant/orders/[id]/status` exige sesión y llama a la función con el token del usuario, sin usar clave de administrador en el navegador. La bandeja `/piloto` ya ofrece ambos botones; el cliente verá el estado actualizado en su consulta periódica. Antes de probar, ejecutar las migraciones 001–005 en orden y asignar membresías verificadas. No se han probado contra una instancia Supabase real. Aún faltan catálogo y precios de servidor, operaciones, repartidor y cobros.

### Operaciones en el piloto (solo lectura)
La migración `202609230006_operations_pilot.sql` crea cuentas autorizadas para Operaciones; solo un administrador puede asignarlas tras verificar identidad. `GET /api/operations/orders` comprueba membresía con el token de la cuenta y consulta pedidos con RLS, sin exponer la clave de servicio. `/piloto` incluye su bandeja sin direcciones y con actualización periódica. Las migraciones 001–006 deben aplicarse en orden en una instancia real antes de probar. **No existe aún asignación de repartidor compartida, pagos ni un catálogo con precios verificados por servidor.**

### Asignación controlada de repartidor (piloto, sin desplegar)
La migración `202609230007_courier_assignment.sql` crea un padrón de repartidores de prueba (alta solo por administrador) y una función transaccional: solo Operaciones puede asignar un pedido `Listo` todavía sin repartidor a un usuario activo del padrón. La bandeja `/piloto` permite elegirlo y asignarlo. El pedido pasa directamente a `En entrega` **solo como simplificación de la prueba**; falta aceptación del repartidor, confirmación de recolección y entrega para el flujo comercial. Ejecutar migraciones 001–007 en orden y registrar cuentas verificadas antes de usarlo. No enviar pedidos reales ni direcciones personales durante estas pruebas.

### Entrega del repartidor (piloto, no comercial)
La migración `202609230008_courier_delivery.sql` permite que exclusivamente el repartidor activo asignado a un pedido `En entrega` lo marque `Entregado`. `GET /api/courier/orders` limita la bandeja a sus propios pedidos y `POST /api/courier/orders/[id]/deliver` invoca la función protegida. `/piloto` ya muestra los pedidos y un botón de confirmación. **Aplicar migraciones 001–008 y configurar el despliegue antes de probar.** No hay aceptación, recolección, prueba de entrega, GPS, pagos ni validación de precios de catálogo en servidor: usar solo pedidos y direcciones ficticios.

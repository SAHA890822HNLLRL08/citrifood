...
export default function Pruebas(){
 const[orders,setOrders]=useState([]),[promos,setPromos]=useState({}),[message,setMessage]=useState(""),[health,setHealth]=useState(null);
 ...
 useEffect(()=>{let active=true;fetch("/api/health",{cache:"no-store"}).then(r=>{if(!r.ok)throw Error("health");return r.json()}).then(data=>{if(active)setHealth(data)}).catch(()=>{if(active)setHealth({error:true})});return()=>{active=false}},[]);

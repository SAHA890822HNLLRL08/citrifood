...
export async function POST(request,{params}){
 ...
 if(!auth)return Response.json({error:"Inicia sesión para asignar repartidores."},{status:401,headers});
 let body;
 try{body=await request.json()}catch{return Response.json({error:"Solicitud inválida."},{status:400,headers})}
 if(!uuid.test(body?.courierId||""))return Response.json({error:"Repartidor inválido."},{status:400,headers});
 try{
  const response=await fetch(auth.config.url+"/rest/v1/rpc/cf_operations_assign_courier",{
   method:"POST",
   headers:{apikey:auth.config.anon,Authorization:"Bearer "+token,"Content-Type":"application/json",Accept:"application/json"},
   body:JSON.stringify({p_order_id:orderId,p_courier_id:body.courierId}),
   signal:AbortSignal.timeout(7000),cache:"no-store",redirect:"error"
  });
  if(!response.ok)return Response.json({error:"No se pudo asignar: revisa permisos, repartidor y estado Listo."},{status:response.status===401||response.status===403?403:409,headers});

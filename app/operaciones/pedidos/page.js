...
export default function Pedidos(){
 ...
 return <main className="ordersAdmin"><header><a href="/operaciones">← Operaciones</a><b>CitriFood · Pedidos</b></header><h1>Pedidos</h1><p>Pedidos del navegador y ejemplos identificados. Asignación manual de demostración. Los pedidos cancelados quedan visibles en el historial.</p>
 <div className="filters">{statuses.map(x=><button className={filter===x?"on":""} onClick={()=>setFilter(x)} key={x}>{x}</button>)}</div>
 <section>{rows.length===0&&<p>No hay pedidos con este estado.</p>}{rows.map(x=><article key={x.id}><div><small>{x.id}{x.id.startsWith("CF-DEMO-")?" · EJEMPLO":""}</small><h2>{x.restaurant}</h2><p>{x.customer} · {x.paymentMethod||x.payment} · $ {x.total}</p></div><div><strong>{x.status}</strong><small>{x.courier||"Sin asignar"}</small>{!x.id.startsWith("CF-DEMO-")&&["Listo","Esperando repartidor"].includes(x.status)&&<button type="button" onClick={()=>updateOrder(x.id,{status:"En entrega",courier:"Juan Pérez (demo)"})}>Asignar a Juan (demo)</button>}</div></article>)}</section></main>
}
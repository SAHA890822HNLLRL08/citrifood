   ...
   {rows.map(x=><article key={x.id}>
    ...
    {expanded===x.id&&<div className="orderDetails">
     {x.id.startsWith("CF-DEMO-")?<p>Esta tarjeta es ilustrativa y no contiene datos de un cliente real.</p>:<>
      ...
      {x.deliveryIssue&&<div className="opsIssue"><h3>⚠️ Incidencia de entrega</h3><p>{x.deliveryIssue.description}</p><small>Reportada: {new Date(x.deliveryIssue.reportedAt).toLocaleString("es-MX")}</small>{x.deliveryIssue.resolvedAt?<p>Atendida: {new Date(x.deliveryIssue.resolvedAt).toLocaleString("es-MX")}</p>:<button type="button" onClick={()=>resolveDeliveryIssue(x.id)}>Marcar como atendida (demo)</button>}</div>}<h3>Productos</h3>
